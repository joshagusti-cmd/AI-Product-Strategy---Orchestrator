-- Lets the prototype's "Reset demo data" button put the shared database
-- back to its seeded state on demand (useful for repeat demos), without
-- exposing a raw DDL/truncate surface to the anon key.
...
begin
  ...
  delete from public.shadow_tools;
