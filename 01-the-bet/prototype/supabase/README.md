...
- `functions/orchestrate/index.ts` — a Deno Edge Function that makes one real Anthropic API call (forced tool-use for structured JSON output) simulating a realistic pass by the Command Center's six agents. **Requires a signed-in session** (see below) — it inspects the caller's JWT `role` claim and rejects anonymous requests with a 401, since this is a real, metered API call. It also enforces each workspace's rolling-24h Orchestrate cap server-side, using the service role key to resolve the caller's workspace and log usage — a client can't raise its own limit or erase its usage history to dodge it.
