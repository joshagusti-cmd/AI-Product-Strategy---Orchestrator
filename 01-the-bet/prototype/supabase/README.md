...
- **Teams/invites are real now, but flat** — a member invites a teammate by email and they join automatically; there's no invite expiry, no re-sending a revoked invite without creating a fresh one, and no way to change an existing member's `role` string from the UI (it's set once, at invite/signup time, and isn't read by any RLS policy — see next point).
- **No RBAC within a workspace** — every member (invited or original) is effectively an "owner": any member can invite, revoke an invite, or remove *another* member (not themselves), regardless of the `role` column's value. The Guardrails doc's governance model (analyst vs. compliance-owner vs. admin) isn't enforced at the auth layer yet, only in the UI's labeling.
