...
- **Real teams/invites** — a workspace member can invite a teammate by email (the "Team" button in the auth widget); they join that workspace automatically the next time they sign in with that email, and a user who belongs to more than one workspace gets a switcher to pick which one is active. Not built yet: RBAC (every member can invite/remove others regardless of role) and leaving a workspace.
...
- **Teams are real but flat, no RBAC** — a workspace member can invite a teammate by email and they join automatically, and a user can belong to multiple workspaces with a switcher between them. What's still open: different roles (analyst vs. compliance owner vs. admin) aren't enforced at the auth layer — any member can invite, revoke an invite, or remove another member. See `supabase/README.md` for the exact gaps.
...
- **Model routing is real but Claude-only** — reassigning an agent's model in the Command Center now actually changes which model runs that agent's real call (see `resolveAgentModel` in `supabase/functions/orchestrate/index.ts`), not just a cosmetic label. Only "Claude Sonnet 5" and "Claude Opus 4.8" are real, though — no OpenAI or Google key is configured, so picking "GPT-4o" or "Gemini 1.5 Pro" falls back to that agent's Claude default and surfaces a toast naming the substitution. True multi-provider support is the natural next step — see `06-the-pitch/roadmap.md`'s "cost-based automatic model routing" item.
...
Closing these is the rest of Horizon 2: RBAC within a workspace, the integrations catalog, and plan-tied spend controls.
