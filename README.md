...
- **Confidence:** M
- **Prototype:** a multi-page governance platform demo lives in this repo at [`01-the-bet/prototype/`](01-the-bet/prototype/) — Command Center, Agent Registry, Policy Editor, Approval Queue, Audit Trail, Workflow History, Model Spend Dashboard, Shadow AI Audit, and Governance Snapshot, backed by a **real Supabase Postgres database**, **real multi-tenant auth with teams and RBAC** (email magic-link sign-in, an isolated private workspace auto-provisioned per user, real email invites with a chosen role so a member can add a teammate to their workspace, three roles enforced by RLS — Admin, Compliance Owner, Analyst — not just labeled, a read-only public demo for anonymous visitors), and **six real, independent Claude API calls with real model routing, manual or automatic** (one call per agent, each seeing the prior agents' actual output; by default a real risk-keyword + scope-size cascade picks each agent's model, escalating to Opus 4.8 only where warranted, with a manual per-agent override available) powering the Command Center's orchestration run, gated to signed-in users and capped by a per-workspace rolling-24h spend limit tied to a real plan tier (Free/Pro/Enterprise → 20/100/500 runs/day, admin-changeable, no real billing behind it yet) enforced server-side, with **real, measured spend telemetry** (every real call's actual model + token usage, priced at real Anthropic rates, on the Model Spend Dashboard) replacing modeled cost-curve numbers for that part of the dashboard, and a **Policy Editor that actually gates the pipeline** — set the workspace's Core-tier policy to "Approval-required" or "Two-gate" and a run whose Risk & Compliance Agent flag crosses the threshold genuinely pauses before the Strategy/Writer calls (and their cost) happen, waiting on a real approval to resume or a real rejection to end it. The **Shadow AI Audit's scan is real too now** — it finds actual Agent Registry drift and real ungoverned-model requests from this workspace's own Orchestrate telemetry, not a fixed list of fictional findings (though it still can't see outside tool use like a personal ChatGPT account, which would need real endpoint/CASB integration this prototype doesn't have). Every completed run is also archived in full now — a new **Workflow History** page lets you search and reopen any past run's real deliverable, not just the Audit Trail's one-line pointer to it. The **Audit Trail can now answer real questions** too — "Ask the audit trail" is a real Claude call grounded strictly in the workspace's own real log entries, gated to signed-in users and rate-limited server-side. The Command Center can also **save the current objective as a reusable template** — scope, auto-route setting, and per-agent model picks included — and load it back in later. The Agent Registry, Approval Queue, and Workflow History are now **filterable by real department**, built from whatever departments actually show up in each page's own data (see `01-the-bet/prototype/supabase/`). There's now a **real mobile companion for the Approval Queue** too (`mobile-approvals.html`) — a genuinely phone-first page, not a squeezed desktop table, reading a lean dedicated query and deciding through the same real approve/reject/resume path as the full console. **Workflow History can now compare two real runs side by side** — check any two archived runs and see both full deliverables juxtaposed, closing the other half of the "workflow history" backlog item. **The Orchestrate pipeline is now data-driven** — it reads its real agents from the workspace's own Agent Registry instead of a hardcoded array, so editing the registry (a real Enabled toggle, for now) genuinely changes what a real run does. There's also now a **universal external-agent connector** — any agent a workspace already runs elsewhere, any stack, gets a real API key and reports its own real activity into the same registry, Audit Trail, and Approval Queue an internal agent uses, with its own irreversible actions genuinely gated by the same real policy if that agent's own code checks in before acting. A **workspace-wide emergency stop** now sits above that — one admin-flipped switch (Workspace modal) that freezes every new Orchestrate run, every paused-run resume, and every external agent's approval check at once, the bigger, blunter sibling of Agent Registry's per-agent Enabled toggle; it's real for everything Aiven itself runs, and still can't reach into an external agent's own code that never checks in first. Every real run now also gets **real PII/secrets redaction** — a deterministic pattern-match pass masks SSNs, emails, phone numbers, credit-card numbers, and API-key-shaped strings out of the objective before any agent sees it and out of every agent's output before it's stored or returned (the external connector runs the identical pass on whatever an outside agent reports), with every masked value counted, surfaced to the user, and logged to the real Audit Trail. The objective also now gets **real prompt-injection screening** — a deterministic pattern match neutralizes override/extraction phrasing (imperative instruction overrides, system-prompt extraction attempts, chat-template injection markers, jailbreak framing) before it's ever prompted to an agent, logging a real, high-severity audit entry the moment it fires. Orchestrate is also now capped by **real dollars, not just run count** — a plan-tied `daily_spend_cap_usd` checked alongside the existing rolling-24h run cap, summing real per-model Anthropic spend from the same telemetry the Model Spend Dashboard reads, so a workspace burning real budget through unusually complex objectives gets stopped even at a normal run count. There's now a **real, workspace-configured webhook** too — an admin pastes a plain URL into the Workspace modal, no OAuth app needed, and the moment a run actually pauses for approval Aiven POSTs a real JSON payload to it (workspace name, objective, risk severity/text, approval id), with every delivery attempt logged and shown as a real success/failure status — nobody has to actively watch the Approval Queue to know a run is waiting. **Workflow History now shows full prompt visibility per step too** — a "View prompt" toggle on each agent step reveals the exact real system/user prompt that agent's Claude call actually received, captured at call time, closing the audit-depth gap of only ever showing the agent's response. The **Shadow AI Audit's scan now also does real anomaly detection** — a real, deterministic statistical check (a 3-sigma outlier against each agent's own rolling token-usage baseline in this workspace's real telemetry, not trained ML) flags any single real call that spiked sharply, feeding the same real finding queue. There's now **real, workspace-configurable data retention** too — an admin sets an auto-delete window (or leaves it unset, the default) and real Audit Trail/Workflow History rows older than that window actually get deleted, enforced both by a real daily `pg_cron` job and an admin-only "Run retention now" button for real, immediate proof it works — a common real GDPR/data-minimization procurement ask. A new **Governance Snapshot** page bundles this workspace's real policies, approval record, and audit trail into one report — formatted for an actual auditor or customer security review, exportable as a real, chrome-stripped print/PDF or as copied Markdown, distinct from the Audit Trail's own raw JSON/CSV export. **True multi-provider routing is now real, bring-your-own-key** — a workspace admin can connect their own real OpenAI or Google API key (Workspace modal → Provider Keys), encrypted at rest via Supabase Vault and never re-displayed once saved; the moment a key is connected, "GPT-4o"/"Gemini 1.5 Pro" genuinely call that provider instead of silently falling back to Claude, and the Shadow AI Audit stops flagging that provider as drift. Aiven itself holds no platform-wide OpenAI/Google credential — this is real multi-provider routing exactly for the workspaces that bring their own key, reframing that gap rather than papering over it. A living **[User Guide](01-the-bet/prototype/USER_GUIDE.md)** walks through every page for someone actually using the console, updated alongside each feature as it ships. Open `01-the-bet/prototype/index.html` directly in a browser, or see it live via GitHub Pages once enabled (Settings → Pages → Source: "GitHub Actions" — see `.github/workflows/deploy-pages.yml`). An earlier single-page version is also published as a Claude Artifact: https://claude.ai/code/artifact/35c46725-ea55-4b4b-9a32-f4ab0388bf50
