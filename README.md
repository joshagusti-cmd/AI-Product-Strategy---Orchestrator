...
- **Confidence:** M
- **Prototype:** a multi-page governance platform demo lives in this repo at [`01-the-bet/prototype/`](01-the-bet/prototype/) — Command Center, Agent Registry, Policy Editor, Approval Queue, Audit Trail, Model Spend Dashboard, and Shadow AI Audit, now backed by a **real, shared Supabase Postgres database** (not localStorage) and a **real Claude API call** powering the Command Center's orchestration run (see `01-the-bet/prototype/supabase/`). Open `01-the-bet/prototype/index.html` directly in a browser, or see it live via GitHub Pages once enabled (Settings → Pages → Source: "GitHub Actions" — see `.github/workflows/deploy-pages.yml`). An earlier single-page version is also published as a Claude Artifact: https://claude.ai/code/artifact/35c46725-ea55-4b4b-9a32-f4ab0388bf50
