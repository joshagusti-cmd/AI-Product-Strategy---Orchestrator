...
- **Confidence:** M
- **Prototype:** a multi-page governance platform demo lives in this repo at [`01-the-bet/prototype/`](01-the-bet/prototype/) — Command Center, Agent Registry, Policy Editor, Approval Queue, Audit Trail, Model Spend Dashboard, and Shadow AI Audit, backed by a **real Supabase Postgres database**, **real multi-tenant auth with teams** (email magic-link sign-in, an isolated private workspace auto-provisioned per user, real email invites so a member can add a teammate to their workspace, a read-only public demo for anonymous visitors), and **six real, independent Claude API calls with real per-agent model routing** (one call per agent, each seeing the prior agents' actual output, on whichever real Claude model the Command Center's model dropdown assigns it) powering the Command Center's orchestration run, gated to signed-in users and capped by a per-workspace rolling-24h spend limit enforced server-side (see `01-the-bet/prototype/supabase/`). Open `01-the-bet/prototype/index.html` directly in a browser, or see it live via GitHub Pages once enabled (Settings → Pages → Source: "GitHub Actions" — see `.github/workflows/deploy-pages.yml`). An earlier single-page version is also published as a Claude Artifact: https://claude.ai/code/artifact/35c46725-ea55-4b4b-9a32-f4ab0388bf50
