...
- ~~Real auth + multi-tenant workspace model (SSO/RBAC)~~ — **built, partially**: email magic-link sign-in with a real per-user private workspace, auto-provisioned and RLS-isolated, plus a read-only public demo for anonymous visitors (`01-the-bet/prototype/supabase/migrations/0003`–`0005`). A workspace member can now invite a teammate by email — they join automatically on their next sign-in, and a user in multiple workspaces gets a switcher (`0007_teams_and_invites.sql`). Not yet built: SSO and RBAC within a workspace (every member can invite/remove others regardless of role). See `01-the-bet/prototype/supabase/README.md`.
