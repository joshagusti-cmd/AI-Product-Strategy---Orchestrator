...
## Platform / real build
- ~~Replace simulated agent runs with real API calls to Claude, GPT, and Gemini behind a common orchestration interface~~ — **built, partially**: the Command Center's orchestration run is six real, independent Claude calls via a Supabase Edge Function (`01-the-bet/prototype/supabase/functions/orchestrate/`), one per agent, each seeing the prior agents' actual output — not one call inventing all six steps, and not yet multi-provider (Claude only; the two agents originally labeled "GPT-4o" now run on and display Claude Sonnet 5). See `01-the-bet/prototype.md`.
