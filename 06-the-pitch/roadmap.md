...
| ~~Ship real auth/multi-tenancy~~ — **built at prototype fidelity**: email magic-link sign-in, an isolated private workspace auto-provisioned per user, a read-only public demo, and a per-workspace rolling-24h spend cap on real Claude calls enforced server-side (`01-the-bet/prototype/supabase/README.md`). Still open, and still gating "first design partner fully self-serve": SSO, teams/invites, RBAC within a workspace, plan-tied spend limits, *automatic* cost-based model routing (manual per-agent routing is now real, see the multi-agent row above), true multi-provider support, and the core integrations catalog (NetSuite, Salesforce, Zendesk, Snowflake, Workday) | First design partner fully self-serve, no manual data wiring | M |
