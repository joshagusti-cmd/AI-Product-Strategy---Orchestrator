...
| ~~Ship real auth/multi-tenancy~~ — **built at prototype fidelity**: email magic-link sign-in, an isolated private workspace auto-provisioned per user, a read-only public demo, and a per-workspace rolling-24h spend cap on real Claude calls enforced server-side (`01-the-bet/prototype/supabase/README.md`). Teams/invites and RBAC within a workspace are also real now — a member invites a teammate by email with a chosen role (Admin/Compliance Owner/Analyst), they join automatically, a multi-workspace user gets a switcher, and each role is enforced by RLS, not just labeled. *Automatic* cost-based model routing is real too — a deterministic risk-keyword + scope-size cascade in the orchestrate Edge Function picks each agent's model by default, escalating to Opus 4.8 only where warranted, with a manual per-agent override still available. Spend limits are plan-tied now too — Free/Pro/Enterprise (20/100/500 Orchestrate runs/day), admin-changeable, though with no real billing/payment behind the plan yet. Model spend is now measured, not modeled — every real Orchestrate call logs its real model and token counts, priced at real Anthropic rates, on the Model Spend Dashboard. The Policy Editor now really gates the pipeline too — a Core-tier policy set to Approval-required/Two-gate genuinely pauses a real run at the Risk & Compliance Agent's flag until a human approves or rejects it, before Strategy/Writer (and their cost) ever run. The pipeline itself is now data-driven, read from the workspace's real Agent Registry rather than a hardcoded array — Agent Registry's new Enabled toggle is a real first lever over it. Still open, and still gating "first design partner fully self-serve": SSO, real billing/payment processing, a trained/scored risk model (today's routing and gate are keyword/severity heuristics, not real risk assessment), true multi-provider support (a bring-your-own-provider-key model per workspace is a promising unlock for this, see `feature-ideas.md`), a universal connector for externally-built agents (in progress), and the core integrations catalog (NetSuite, Salesforce, Zendesk, Snowflake, Workday) | First design partner fully self-serve, no manual data wiring | M |
...
| Everything else still sitting in `feature-ideas.md` and not triaged above — ~~drill-down analytics~~ (built: a real department filter on Agent Registry/Approval Queue/Workflow History), ~~workflow history~~ (built: `workflow-history.html`, now with a real run-to-run comparison view), ~~natural-language audit-trail Q&A~~ (built: `audit.html`'s "Ask the audit trail" panel, a real Claude call grounded in the real log), ~~saved objective templates~~ (built: the Command Center can save/reload a named objective+scope+routing template), ~~mobile approvals~~ (built: `mobile-approvals.html`, a real phone-first companion to the Approval Queue), in-product kill-switch messaging | Revisit once Horizon 1 and 2 are proven, not before | L |
